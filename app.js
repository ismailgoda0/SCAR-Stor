...
        this.state.userInfo = JSON.parse(localStorage.getItem(`scarUserInfo_${storageVersion}`)) || { name: null, phone: null };
...
                googleSheetcomplaintUrl: ""
            };
        }
    },

   // في ملف app.js، استبدل هذه الوظيفة بالكامل
    ...
    async loadStoreData() {
        try {
            ...
            if (!storeData || !storeData.products || !storeData.categories) {
                // حالة إضافية إذا كان الملف JSON صالحًا ولكنه لا يحتوي على البيانات الأساسية
            ...
            this.state.storeData.products = storeData.products.map(product => {
                ...
                if (product.discountPercentage > 0) {
                    const originalPrice = product.basePrice / (1 - (product.discountPercentage / 100));
                    oldPrice = Math.round(originalPrice);
                }
            ...
            if(mainContent){
                 ...
                 mainContent.dataset.error = "true"; // نضع علامة لوجود خطأ
            }
    ...
    async resetAndReloadApp() {
        ...
        // إذا حدث خطأ في التحميل، نتوقف هنا
...
                label: p.name,
                value: p.name,
                image: (p.isBundle && p.images && p.images.length > 0) ? p.images[0] : p.heroImage || (p.isBundle ? this.state.productMap.get(p.items[0].productId)?.images[0] : p.images[0])
            })),
...
            if(localSearchInput) {
                localSearchInput.value = state.currentSearchTerm;
            }
            
            const categoryId = params.get('category');
            if (categoryId) {
...
        },
// الكود المُعدَّل في ملف app.js

navigateTo(url, fromProductPage = false) {
    ScarStore.Modals.closeLast(); // <<-- أضف هذا السطر هنا
...
        
            if (product.isBundle) {
                // This logic needs to be more robust if bundles have selectable variants
...
                    } else {
                        // If no radio is checked, default to the first option
...
            let oldPrice = null;
            if (product.discountPercentage > 0) {
                oldPrice = Math.round(finalPrice / (1 - (product.discountPercentage / 100)));
...
            productsTitle.innerHTML = `${title} <span class="text-lg text-slate-500 font-medium">(${productsToDisplay.length} منتج)</span>`;
...
>>>>>>> 4e67c1773d14065bba5fbdebf9f241385665dabd
